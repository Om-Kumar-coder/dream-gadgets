...
export function ProductBuyPanel({ name, price, originalPrice, imageUrl, productId, imei, slug }: ProductBuyPanelProps) {
  const { addItem } = useCartStore();
  const [justAdded, setJustAdded] = useState(false);
  const [showPanel, setShowPanel] = useState(true);

  const handleAdd = () => {
    addItem({ id: productId, imei: imei || productId, name, price, slug, imageUrl });
    setJustAdded(true);
