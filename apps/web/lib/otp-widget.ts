...
/**
 * Extract the JWT access token from the widget's success callback payload.
 *
 * MSG91 documents the token as `data.accessToken`; some SDK versions have
 * shipped it in `data.message` or `data.token`, so check those as fallbacks.
 * (Reading only message/token — as an earlier integration did — silently
 * dropped the token and broke verification on production.)
 */
export function extractWidgetToken(data: unknown): string | null {
  const d = (data ?? {}) as Record<string, unknown>;
  for (const key of ['accessToken', 'message', 'token', 'access_token']) {
    const value = d[key];
