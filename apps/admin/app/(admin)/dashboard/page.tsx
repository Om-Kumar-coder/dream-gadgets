...
interface StockChartPoint { condition: string; count: number; }
...
export default function DashboardPage() {
  ...
  const isMultiStore = role === 'multi_store_manager';
  ...
  useEffect(() => {
    ...
    unsubs.push(socket.on('sale.voided', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
    unsubs.push(socket.on('inventory.updated', () => {
      qc.invalidateQueries({ queryKey: ['dashboard-kpi'] });
      qc.invalidateQueries({ queryKey: ['dashboard-stock-condition'] });
    }));
    unsubs.push(socket.on('return.created', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
    unsubs.push(socket.on('order.created', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
    unsubs.push(socket.on('order.status_changed', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
    unsubs.push(socket.on('stock.transfer.created', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
    unsubs.push(socket.on('stock.transfer.received', () => qc.invalidateQueries({ queryKey: ['dashboard-kpi'] })));
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div>
          <h1 className="heading-sm text-surface-900">
            {isOwner ? 'Owner Dashboard' :
             isMultiStore ? 'Multi-Store Overview' :
             role === 'store_manager' ? 'Store Dashboard' :
             role === 'shop_sales' || role === 'store_sales' ? 'My Sales Dashboard' :
             role === 'calling_staff' ? 'My Dashboard' :
             'Dashboard'}
          </h1>
      ...
      {/* Charts — visible to managers and above */}
      {isManager && (
            ...
            {stockChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={stockChartData}>
                  ...
                  <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid hsl(0, 0%, 93%)', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }} />
      ...
      {/* Permission Changes + Banner Analytics — owner only */}
      {isOwner && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      {/* Buyback Leads — owner, multi-store manager, store manager */}
      {(isOwner || isMultiStore || role === 'store_manager') && buybackStats && (
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
