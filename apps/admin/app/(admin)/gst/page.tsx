...
function KpiCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-surface-200 p-5 flex items-start gap-4 hover:shadow-sm transition-shadow">
      <div className={`p-2.5 rounded-lg ${color}`}>
        <Icon className="w-5 h-5 text-white" />
      </div>
      <div className="min-w-0">
        <p className="text-xs text-surface-500 font-medium uppercase tracking-wide truncate">{title}</p>
        <p className="text-2xl font-bold text-surface-900 mt-0.5">{value}</p>
        <p className="text-xs text-surface-400 mt-0.5">{count} {typeof count === 'number' ? 'entries' : ''}</p>
      </div>
    </div>
  );
}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-surface-400">{count} {count === 1 ? 'entry' : 'entries'}</p>
          </div>
        </div>
        {open ? (
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-surface-100 bg-surface-50/50">
            {headers.map((h) => (
              <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-surface-500 uppercase tracking-wider whitespace-nowrap">
                {h}
              </th>
            ))}
          </tr>
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-surface-600 mb-1.5">Branch</label>
      ...
      {summary && !isLoading && (
        <div className="card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-surface-400">Total Taxable Value</p>
              <p className="text-lg font-bold text-surface-900">{formatINR(summary.totalTaxableValue)}</p>
            </div>
            <div>
              <p className="text-xs text-surface-400">Total CGST</p>
              <p className="text-lg font-bold text-blue-600">{formatINR(summary.totalCgst)}</p>
            </div>
            <div>
              <p className="text-xs text-surface-400">Total SGST</p>
              <p className="text-lg font-bold text-emerald-600">{formatINR(summary.totalSgst)}</p>
            </div>
            <div>
              <p className="text-xs text-surface-400">Total IGST</p>
      ...
      {!isLoading && !isError && data && (
        data.b2b.length === 0 && data.b2cl.length === 0 && data.b2cs.length === 0 && data.cdnr.length === 0 && (
          <div className="card p-12 text-center">
            ...
            <h3 className="text-base font-semibold text-surface-900 mb-1">No GSTR-1 Data</h3>
