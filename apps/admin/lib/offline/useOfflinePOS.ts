...
import { offlineDB, CachedInventoryItem, PendingSale } from './db';
...
export function useOfflinePOS() {
    ...
    async (query: string): Promise<OfflineSearchResult[]> => {
      ...
      if (navigator.onLine) {
        try {
          ...
          if (items.length > 0) {
            const cacheItems: CachedInventoryItem[] = items.map((item: any) => ({
              id: item.id,
              imei: item.imei,
  ...
  const submitSale = useCallback(
    async (salePayload: object): Promise<{ success: boolean; invoiceNumber?: string; queued: boolean; queueId?: number }> => {
      if (navigator.onLine) {
        try {
