...
function generateVariants(modelName, brandName, colors, storages, rams, boxType, condition, purchasePrice, wholesalePrice, sellingPrice, onlinePrice) {
    const items = [];
...
export async function seedProducts(dataSource) {
    ...
    try {
        ...
        for (const brandData of BRANDS) {
            const [existingBrand] = await queryRunner.query('SELECT id FROM brands WHERE name = $1', [brandData.name]);
            if (existingBrand) {
            ...
            const brandId = uuid();
            await queryRunner.query(`INSERT INTO brands (id, name, logo_url, is_active, sort_order, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`, [brandId, brandData.name, brandData.logoUrl, true, 0]);
        ...
        for (const modelData of MODELS) {
            ...
            const [existingModel] = await queryRunner.query('SELECT id FROM models WHERE name = $1 AND brand_id = $2', [modelData.name, brandId]);
            if (existingModel) {
            ...
            const modelId = uuid();
            await queryRunner.query(`INSERT INTO models (id, name, brand_id, slug, description, specs, is_active, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [modelId, modelData.name, brandId, modelData.slug, modelData.description, JSON.stringify(modelData.specs), true]);
        ...
        for (const itemData of INVENTORY_ITEMS) {
            ...
            await queryRunner.query(`INSERT INTO inventory_items (
          ...
          selling_price, online_price, images, status, is_online, branch_id, created_at, updated_at
            ...
            for (let i = 0; i < itemData.photoUrls.length; i++) {
                const photoId = uuid();
                await queryRunner.query(`INSERT INTO item_photos (id, item_id, key, url, sort_order, is_primary, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW())`, [photoId, itemId, `photo-${i + 1}`, itemData.photoUrls[i], i, i === 0]);
            }
            itemCount++;
        }
        ...
        for (const accData of ACCESSORIES) {
            ...
            const accId = uuid();
            await queryRunner.query(`INSERT INTO accessories (id, sku, name, description, brand_id, category, purchase_price, selling_price, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW())`, [accId, accData.sku, accData.name, accData.description, brandId, accData.category, accData.purchasePrice, accData.sellingPrice]);
            accessoryCount++;
        }
...
//# sourceMappingURL=004-seed-products.js.map