...
export async function seedRolesAndPermissions(dataSource) {
    ...
    try {
        ...
        for (const module of MODULES) {
            for (const action of ACTIONS) {
                const result = await queryRunner.query(`INSERT INTO permissions (module, action, description)
           VALUES ($1, $2, $3)
           ON CONFLICT (module, action) DO UPDATE SET description = EXCLUDED.description
           RETURNING id`, [module, action, `${action} ${module}`]);
        ...
        for (const [roleName, modulePerms] of Object.entries(ROLE_PERMISSIONS)) {
            const roleResult = await queryRunner.query(`INSERT INTO roles (name, description, is_system)
         VALUES ($1, $2, true)
         ON CONFLICT (name) DO UPDATE SET description = EXCLUDED.description
         RETURNING id`, [roleName, roleName.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())]);
            ...
            for (const [module, actions] of Object.entries(modulePerms)) {
                for (const action of actions) {
                    ...
                    if (permId) {
                        await queryRunner.query(`INSERT INTO role_permissions (role_id, permission_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [roleId, permId]);
                    }
                }
            }
        }
...
//# sourceMappingURL=001-seed-roles-permissions.js.map