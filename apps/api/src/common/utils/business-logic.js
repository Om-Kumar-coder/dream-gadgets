...
export function calculateExchangePrice(basePrice, batteryHealth, monthsSinceFirstInvoice) {
...
export function calculateWarrantyExpiry(firstInvoiceDate, condition) {
    ...
    if (!months || !firstInvoiceDate)
        return null;
    const d = new Date(firstInvoiceDate);
    d.setMonth(d.getMonth() + months);
    return d;
}
export function getRequiredDiscountRole(discountPercent) {
...
export function getRequiredReturnRole(returnAmount) {
    if (returnAmount < 5000)
        return 'any';
    if (returnAmount <= 25000)
        return 'manager';
    return 'owner';
}
