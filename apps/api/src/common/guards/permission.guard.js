...
let PermissionGuard = (() => {
    ...
    var PermissionGuard = _classThis = class {
        constructor(reflector) {
            ...
            this.logger = new Logger(PermissionGuard.name);
        }
        canActivate(context) {
            const required = this.reflector.getAllAndOverride(PERMISSION_KEY, [
                context.getHandler(),
                context.getClass(),
            ]);
            if (!required)
            ...
            if (!hasPermission) {
                this.logger.warn(`Permission denied: ${method} ${path} | user=${user?.sub ?? 'unknown'} role=${user?.role ?? 'none'} missing=${required}`);
