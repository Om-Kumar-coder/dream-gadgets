...
describe('ClientService', () => {
    ...
    describe('create()', () => {
        ...
        it('should create a client with valid data', async () => {
            ...
            clientRepo.findOne.mockResolvedValue(null); // no duplicate
        ...
        it('should reject with PHONE_DUPLICATE if phone already exists', async () => {
            clientRepo.findOne.mockResolvedValue(makeClient()); // duplicate found
        ...
        it('should set ekycStatus to pending on creation', async () => {
            ...
            expect(clientRepo.create).toHaveBeenCalledWith(expect.objectContaining({ ekycStatus: 'pending' }));
        });
        it('should set isActive to true on creation', async () => {
            ...
            clientRepo.save.mockResolvedValue(savedClient);
            await service.create(baseDto, 'user-1');
            expect(clientRepo.create).toHaveBeenCalledWith(expect.objectContaining({ isActive: true }));
        });
        it('should convert dateOfBirth string to Date', async () => {
            ...
            const createCall = clientRepo.create.mock.calls[0][0];
    ...
    describe('findAll()', () => {
        ...
        it('should apply search filter when provided', async () => {
            ...
            expect(qb.andWhere).toHaveBeenCalledWith(expect.stringContaining('ILIKE'), expect.objectContaining({ search: '%John%' }));
        });
        it('should apply branchId filter when provided', async () => {
            const qb = makeQueryBuilder([], 0);
            clientRepo.createQueryBuilder.mockReturnValue(qb);
            await service.findAll({ branchId: 'branch-1' });
            expect(qb.andWhere).toHaveBeenCalledWith('client.branchId = :branchId', { branchId: 'branch-1' });
        });
        it('should apply customerType filter when provided', async () => {
            const qb = makeQueryBuilder([], 0);
            clientRepo.createQueryBuilder.mockReturnValue(qb);
            await service.findAll({ customerType: 'corporate' });
            expect(qb.andWhere).toHaveBeenCalledWith('client.customerType = :customerType', { customerType: 'corporate' });
        });
    });
    // ─── 8.2: Update client ───────────────────────────────────────────────────
    describe('update()', () => {
        it('should update client fields', async () => {
            ...
            clientRepo.findOne.mockResolvedValue(client);
            clientRepo.save.mockResolvedValue(updated);
    ...
    describe('uploadEkycDocuments()', () => {
        it('should store documents and set ekycStatus to pending', async () => {
            ...
            clientRepo.findOne.mockResolvedValue(client);
            clientRepo.save.mockResolvedValue({ ...client, ekycDocuments: docs, ekycStatus: 'pending' });
            const result = await service.uploadEkycDocuments('client-uuid-1', docs);
            expect(clientRepo.save).toHaveBeenCalledWith(expect.objectContaining({ ekycDocuments: docs, ekycStatus: 'pending' }));
        });
        it('should throw NotFoundException when client not found', async () => {
            clientRepo.findOne.mockResolvedValue(null);
    ...
    describe('verifyEkyc()', () => {
        it('should set ekycStatus to verified with timestamp and userId', async () => {
            ...
            clientRepo.findOne.mockResolvedValue(client);
            clientRepo.save.mockResolvedValue({
                ...client,
                ekycStatus: 'verified',
            ...
            expect(clientRepo.save).toHaveBeenCalledWith(expect.objectContaining({
                ekycStatus: 'verified',
                ekycVerifiedById: 'user-1',
            }));
            const saveArg = clientRepo.save.mock.calls[0][0];
