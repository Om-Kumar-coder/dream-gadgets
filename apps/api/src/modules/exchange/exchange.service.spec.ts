...
function makeExchange(overrides: Partial<ExchangeDevice> = {}): ExchangeDevice {
  return {
    id: 'exchange-uuid-1',
    clientId: 'client-1',
    saleId: null,
...
describe('ExchangeService', () => {
  ...
  describe('create()', () => {
    ...
    it('should create an exchange with valid data', async () => {
      ...
      (exchangeRepo.create as any).mockReturnValue(savedExchange);
      (exchangeRepo.save as any).mockResolvedValue(savedExchange);
    ...
    it('should set addedToInventory to false on creation', async () => {
      ...
      expect(exchangeRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ addedToInventory: false }),
      );
    });
    ...
    it('should store condition assessment notes', async () => {
      ...
      expect(exchangeRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ conditionNotes }),
      );
    });
  });
  ...
  describe('findById()', () => {
    it('should return exchange when found', async () => {
      ...
      (exchangeRepo.findOne as any).mockResolvedValue(exchange);
    ...
    it('should throw NotFoundException when exchange not found', async () => {
      (exchangeRepo.findOne as any).mockResolvedValue(null);
  ...
  describe('findAll()', () => {
    function makeQueryBuilder(data: ExchangeDevice[], total: number): any {
      const qb: any = {
    ...
    it('should return paginated list', async () => {
      ...
      (exchangeRepo.createQueryBuilder as any).mockReturnValue(qb);
    ...
    it('should apply addedToInventory filter', async () => {
      ...
      expect(qb.andWhere).toHaveBeenCalledWith(
        'exchange.addedToInventory = :addedToInventory',
        { addedToInventory: false },
      );
    });
  });
  ...
  describe('update()', () => {
    it('should update exchange fields', async () => {
      ...
      (exchangeRepo.findOne as any).mockResolvedValue(exchange);
      (exchangeRepo.save as any).mockResolvedValue(updated);
  ...
  describe('addToInventory()', () => {
    it('should create inventory item and update exchange record', async () => {
      ...
      (itemRepo.create as any).mockReturnValue(newItem);
      (itemRepo.save as any).mockResolvedValue(newItem);
      (exchangeRepo.save as any).mockResolvedValue({ ...exchange, addedToInventory: true, inventoryItemId: 'new-item-uuid' });

      const result = await service.addToInventory('exchange-uuid-1', { branchId: 'branch-1' });

      expect(itemRepo.save).toHaveBeenCalled();
      expect(exchangeRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ addedToInventory: true }),
      );
    });
    ...
    it('should throw BadRequestException if already added to inventory', async () => {
      ...
      await expect(
        service.addToInventory('exchange-uuid-1', { branchId: 'branch-1' }),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.addToInventory('exchange-uuid-1', { branchId: 'branch-1' }),
      ).rejects.toMatchObject({ response: { code: 'ALREADY_IN_INVENTORY' } });
    });
    ...
    it('should throw NotFoundException when exchange not found', async () => {
      ...
      await expect(
        service.addToInventory('non-existent', { branchId: 'branch-1' }),
    ...
    it('should use exchangePrice as purchasePrice when not provided', async () => {
      ...
      (itemRepo.create as any).mockReturnValue(newItem);
      (itemRepo.save as any).mockResolvedValue(newItem);
      (exchangeRepo.save as any).mockResolvedValue({ ...exchange, addedToInventory: true });

      await service.addToInventory('exchange-uuid-1', { branchId: 'branch-1' });

      expect(itemRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ purchasePrice: 7500 }),
      );
    });
  });
  ...
  describe('getPriceGuide()', () => {
    it('should return empty array when table does not exist', async () => {
      const mockQb: any = {
        ...
        getRawMany: (jest.fn() as any).mockRejectedValue(new Error('relation does not exist')),
      };
      (exchangeRepo.manager.createQueryBuilder as any).mockReturnValue(mockQb);
    ...
    it('should return price guide data when table exists', async () => {
      ...
      const mockQb: any = {
        ...
        from: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        getRawMany: (jest.fn() as any).mockResolvedValue(priceGuideData),
      };
      (exchangeRepo.manager.createQueryBuilder as any).mockReturnValue(mockQb);
