...
// ─── Mock DataSource ──────────────────────────────────────────────────────────
function makeDataSource(queryResult = []) {
    return {
...
describe('ReportService', () => {
    ...
    describe('generatePdf()', () => {
        it('should throw for unsupported report types', async () => {
            await expect(service.generatePdf('stock_aging', {})).rejects.toThrow(BadRequestException);
            await expect(service.generatePdf('stock_aging', {})).rejects.toMatchObject({ response: { code: 'PDF_NOT_SUPPORTED' } });
        });
