...
function makeSale(overrides = {}) {
    return {
        ...
        voidedById: null,
        voidedAt: null,
        items: [{ id: 'si-1', itemId: 'item-1', saleId: 'sale-uuid-1' }],
        payments: [],
        createdAt: new Date(),
        updatedAt: new Date(),
        ...overrides,
...
describe('ReturnService', () => {
    ...
    describe('createSaleReturn()', () => {
        ...
        it('should create a sale return within the return window', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
        ...
        it('should throw NotFoundException when sale not found', async () => {
            ...
            await expect(service.createSaleReturn('non-existent', baseDto, 'user-1', 'sales')).rejects.toThrow(NotFoundException);
        });
        it('should throw BadRequestException for voided sale', async () => {
            const sale = makeSale({ isVoided: true });
            saleRepo.findOne.mockResolvedValue(sale);
            await expect(service.createSaleReturn('sale-uuid-1', baseDto, 'user-1', 'sales')).rejects.toThrow(BadRequestException);
        });
        ...
        it('should reject return outside window without manager override', async () => {
            ...
            saleRepo.findOne.mockResolvedValue(sale);
            await expect(service.createSaleReturn('sale-uuid-1', baseDto, 'user-1', 'sales')).rejects.toThrow(BadRequestException);
            await expect(service.createSaleReturn('sale-uuid-1', baseDto, 'user-1', 'sales')).rejects.toMatchObject({ response: { code: 'RETURN_WINDOW_EXPIRED' } });
        });
        it('should allow return outside window with managerOverride=true', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            const result = await service.createSaleReturn('sale-uuid-1', { ...baseDto, managerOverride: true }, 'user-1', 'store manager');
            expect(result).toBeDefined();
        });
        ...
        it('should reject high-value return (>25000) for non-owner', async () => {
            ...
            saleRepo.findOne.mockResolvedValue(sale);
            await expect(service.createSaleReturn('sale-uuid-1', { ...baseDto, refundAmount: 30000 }, 'user-1', 'sales')).rejects.toThrow(ForbiddenException);
        });
        it('should allow high-value return (>25000) for owner', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            const result = await service.createSaleReturn('sale-uuid-1', { ...baseDto, refundAmount: 30000 }, 'user-1', 'shop owner');
            expect(result).toBeDefined();
        });
        it('should require manager for return between 5000 and 25000', async () => {
            const sale = makeSale({ totalAmount: 10000, saleDate: new Date() });
            saleRepo.findOne.mockResolvedValue(sale);
            await expect(service.createSaleReturn('sale-uuid-1', { ...baseDto, refundAmount: 10000 }, 'user-1', 'sales')).rejects.toThrow(ForbiddenException);
        });
        it('should allow any staff for return below 5000', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            const result = await service.createSaleReturn('sale-uuid-1', { ...baseDto, refundAmount: 3000 }, 'user-1', 'sales');
            expect(result).toBeDefined();
        });
        ...
        it('should update inventory items to "scrapped" when conditionAssessment is scrapped', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            await service.createSaleReturn('sale-uuid-1', { ...baseDto, conditionAssessment: 'scrapped' }, 'user-1', 'sales');
        ...
        it('should set refundStatus to "processed" for original_payment method', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            const result = await service.createSaleReturn('sale-uuid-1', { ...baseDto, refundMethod: 'original_payment' }, 'user-1', 'sales');
            // Verify create was called with processed status
            const createCall = returnRepo.create.mock.calls[0][0];
    ...
    describe('createPurchaseReturn()', () => {
        ...
        it('should throw NotFoundException when purchase not found', async () => {
            ...
            await expect(service.createPurchaseReturn('non-existent', baseDto, 'user-1')).rejects.toThrow(NotFoundException);
        });
        it('should return only specified items when itemIds provided', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            await service.createPurchaseReturn('purchase-uuid-1', { ...baseDto, itemIds: ['item-2'] }, 'user-1');
        ...
        it('should throw NotFoundException when specified items not found', async () => {
            const purchase = makePurchase();
            purchaseRepo.findOne.mockResolvedValue(purchase);
            itemRepo.find.mockResolvedValue([]); // none found — length < itemIds.length
            await expect(service.createPurchaseReturn('purchase-uuid-1', { ...baseDto, itemIds: ['item-missing'] }, 'user-1')).rejects.toThrow(NotFoundException);
        });
        it('should set conditionAssessment to "available" when specified', async () => {
            ...
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            await service.createPurchaseReturn('purchase-uuid-1', { ...baseDto, conditionAssessment: 'available' }, 'user-1');
            expect(itemRepo.update).toHaveBeenCalledWith('item-1', { status: 'available' });
        });
        it('should use In() operator for itemIds lookup', async () => {
            ...
            const savedReturn = makeReturn({ returnType: 'purchase' });
            purchaseRepo.findOne.mockResolvedValue(purchase);
            itemRepo.find.mockImplementation(({ where }) => {
                // Verify the In() function was used (it returns an object with _type/_value)
                expect(where.id).toBeDefined();
                return Promise.resolve(items.filter((i) => where.id._value?.includes(i.id)));
            });
            itemRepo.update.mockResolvedValue({ affected: 1 });
            returnRepo.create.mockReturnValue(savedReturn);
            returnRepo.save.mockResolvedValue(savedReturn);
            await service.createPurchaseReturn('purchase-uuid-1', { ...baseDto, itemIds: ['item-1', 'item-2'] }, 'user-1');
            expect(itemRepo.find).toHaveBeenCalledWith(expect.objectContaining({
                where: expect.objectContaining({
                    id: expect.any(Object),
                }),
    ...
    describe('findAll()', () => {
        ...
        it('should apply returnType filter', async () => {
            ...
            expect(qb.andWhere).toHaveBeenCalledWith('ret.returnType = :returnType', { returnType: 'sale' });
        });
    });
});
//# sourceMappingURL=return.service.spec.js.map