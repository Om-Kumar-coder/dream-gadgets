...
function generateValidIMEI(base) {
...
describe('InventoryService', () => {
    ...
    describe('create()', () => {
        ...
        it('should compute totalCost = purchasePrice + taxAmount', async () => {
            ...
            expect(itemRepo.create).toHaveBeenCalledWith(expect.objectContaining({ totalCost: 11800 }));
        ...
        it('should set status to "available" on creation', async () => {
            ...
            expect(itemRepo.create).toHaveBeenCalledWith(expect.objectContaining({ status: 'available' }));
        });
        it('should set warrantyExpiry to 12 months for sealed_pack', async () => {
            ...
            expect(itemRepo.create).toHaveBeenCalledWith(expect.objectContaining({ warrantyExpiry: expectedExpiry }));
        });
        ...
        it('should set warrantyExpiry to null for mint condition', async () => {
            ...
            await service.create(dto, 'user-1');
            expect(itemRepo.create).toHaveBeenCalledWith(expect.objectContaining({ warrantyExpiry: null }));
        });
    ...
    describe('update() - status transitions', () => {
        it('should allow valid transition: available → sold', async () => {
            ...
            itemRepo.save.mockResolvedValue({ ...item, status: 'sold' });
            dataSource.query.mockResolvedValue([]);
            const result = await service.update(item.id, { status: 'sold' }, 'user-1');
            expect(result.status).toBe('sold');
        });
        it('should allow valid transition: available → booked', async () => {
            const item = makeItem({ status: 'available' });
            itemRepo.findOne.mockResolvedValue(item);
            itemRepo.save.mockResolvedValue({ ...item, status: 'booked' });
            dataSource.query.mockResolvedValue([]);
            const result = await service.update(item.id, { status: 'booked' }, 'user-1');
            expect(result.status).toBe('booked');
        });
        it('should reject invalid transition: available → returned', async () => {
            const item = makeItem({ status: 'available' });
            itemRepo.findOne.mockResolvedValue(item);
            await expect(service.update(item.id, { status: 'returned' }, 'user-1')).rejects.toThrow(BadRequestException);
            await expect(service.update(item.id, { status: 'returned' }, 'user-1')).rejects.toMatchObject({ response: { code: 'INVALID_STATUS_TRANSITION' } });
        });
        it('should reject invalid transition: sold → available', async () => {
            const item = makeItem({ status: 'sold' });
            itemRepo.findOne.mockResolvedValue(item);
            await expect(service.update(item.id, { status: 'available' }, 'user-1')).rejects.toThrow(BadRequestException);
        });
        ...
        it('should allow valid transition: sold → returned', async () => {
            const item = makeItem({ status: 'sold' });
            itemRepo.findOne.mockResolvedValue(item);
            itemRepo.save.mockResolvedValue({ ...item, status: 'returned' });
            dataSource.query.mockResolvedValue([]);
            const result = await service.update(item.id, { status: 'returned' }, 'user-1');
            expect(result.status).toBe('returned');
        });
        it('should allow valid transition: returned → available', async () => {
            const item = makeItem({ status: 'returned' });
            itemRepo.findOne.mockResolvedValue(item);
            itemRepo.save.mockResolvedValue({ ...item, status: 'available' });
            dataSource.query.mockResolvedValue([]);
            const result = await service.update(item.id, { status: 'available' }, 'user-1');
    ...
    describe('getPriceSuggestion()', () => {
        it('should return median and count from query result', async () => {
            dataSource.query.mockResolvedValue([{ median: '15000.00', count: '5' }]);
        ...
        it('should return null median when no sales data', async () => {
            dataSource.query.mockResolvedValue([{ median: null, count: '0' }]);
            const result = await service.getPriceSuggestion('model-1', 'mint');
    ...
    describe('addPhoto()', () => {
        it('should add a photo when under the 10-photo limit', async () => {
            const item = makeItem();
            itemRepo.findOne.mockResolvedValue(item);
            photoRepo.count.mockResolvedValue(3);
            const photo = {
                id: 'photo-1',
                itemId: item.id,
            ...
            photoRepo.create.mockReturnValue(photo);
            photoRepo.save.mockResolvedValue(photo);
    ...
    describe('deletePhoto()', () => {
        it('should delete photo when found', async () => {
            ...
            photoRepo.remove.mockResolvedValue(photo);
    ...
    describe('toggleOnline()', () => {
        it('should toggle isOnline from false to true', async () => {
            ...
            itemRepo.findOne.mockResolvedValue(item);
            itemRepo.save.mockResolvedValue({ ...item, isOnline: true });
        ...
        it('should toggle isOnline from true to false', async () => {
            ...
            itemRepo.findOne.mockResolvedValue(item);
            itemRepo.save.mockResolvedValue({ ...item, isOnline: false });
            const result = await service.toggleOnline(item.id, 'user-1');
