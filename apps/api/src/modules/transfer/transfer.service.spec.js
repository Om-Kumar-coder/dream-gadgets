...
function makeBranch(id, code) {
    return { id, code, name: `Branch ${code}`, isActive: true };
}
// ─── Mock factories ──────────────────────────────────────────────────────────
...
describe('TransferService', () => {
    ...
    describe('create()', () => {
        ...
        beforeEach(() => {
            branchRepo.findOne.mockResolvedValue(makeBranch('branch-from', 'FROM'));
            const savedTransfer = makeTransfer();
            queryRunner.manager.create.mockReturnValue(savedTransfer);
            queryRunner.manager.save.mockResolvedValue(savedTransfer);
        ...
        it('should throw NotFoundException when items not found', async () => {
            itemRepo.find.mockResolvedValue([]); // no items found
        ...
        it('should set items status to transferred on create', async () => {
            ...
            expect(queryRunner.manager.update).toHaveBeenCalledWith(InventoryItem, item.id, { status: 'transferred' });
        });
        it('should generate a transfer number with TRF- prefix', async () => {
            const item = makeItem({ branchId: 'branch-from', status: 'available' });
            itemRepo.find.mockResolvedValue([item]);
            await service.create(baseDto, 'user-1');
            const createCall = queryRunner.manager.create.mock.calls[0][1];
    ...
    describe('findAll()', () => {
        ...
        it('should apply status filter', async () => {
            ...
            expect(qb.andWhere).toHaveBeenCalledWith('transfer.status = :status', { status: 'received' });
        });
    });
    ...
    describe('receive()', () => {
        it('should update confirmed items to received and move to toBranch', async () => {
            ...
            expect(queryRunner.manager.update).toHaveBeenCalledWith(StockTransferItem, transferItem.id, { status: 'received' });
            expect(queryRunner.manager.update).toHaveBeenCalledWith(InventoryItem, 'item-uuid-1', { branchId: 'branch-to', status: 'available' });
        });
        ...
        it('should set receivedById and receivedAt on transfer', async () => {
            ...
            queryRunner.manager.update.mockResolvedValue({ affected: 1 });
            await service.receive('transfer-uuid-1', ['item-uuid-1'], 'user-2');
            expect(queryRunner.manager.update).toHaveBeenCalledWith(StockTransfer, 'transfer-uuid-1', expect.objectContaining({ status: 'received', receivedById: 'user-2' }));
        });
    });
    // ─── 9.5: Reject transfer ─────────────────────────────────────────────────
    describe('reject()', () => {
        it('should restore all items to available at fromBranch', async () => {
            ...
            expect(queryRunner.manager.update).toHaveBeenCalledWith(InventoryItem, 'item-uuid-1', { branchId: 'branch-from', status: 'available' });
        });
        it('should record rejection reason', async () => {
            ...
            expect(queryRunner.manager.update).toHaveBeenCalledWith(StockTransfer, 'transfer-uuid-1', expect.objectContaining({ status: 'rejected', rejectionReason: 'Damaged items' }));
        });
        it('should reject if transfer is not in initiated or in_transit status', async () => {
            const transfer = makeTransfer({ status: 'received' });
            transferRepo.findOne.mockResolvedValue(transfer);
...
//# sourceMappingURL=transfer.service.spec.js.map