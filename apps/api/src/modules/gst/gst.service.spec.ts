...
import { GstService, B2bEntry, B2clEntry, B2csEntry, CdnrEntry } from './gst.service';
 ...
 */
function b2bRow(overrides: Record<string, any> = {}): any {
  return {
    sale_id: 'sale-uuid-1',
 ...
 */
function saleItemRow(overrides: Record<string, any> = {}): any {
  return {
    sale_id: 'sale-uuid-1',
    hsn_code: '84713000',
    unit_price: '80000',
 ...
 */
function b2clRow(overrides: Record<string, any> = {}): any {
  return {
    sale_id: 'sale-b2cl-1',
 ...
 */
function b2csRow(overrides: Record<string, any> = {}): any {
  return {
    total_amount: '5000',
    subtotal: '4237.29',
    tax_amount: '762.71',
 ...
 */
function cdnrRow(overrides: Record<string, any> = {}): any {
  return {
 ...
 */
function isB2bQuery(sql: string): boolean {
 ...
 */
function isB2clQuery(sql: string): boolean {
 ...
 */
function isB2csQuery(sql: string): boolean {
 ...
 */
function isSaleItemsQuery(sql: string): boolean {
 ...
 */
function isCdnrQuery(sql: string): boolean {
...
describe('GstService', () => {
  ...
  describe('B2B classification (registered customers with GSTIN)', () => {
    it('should classify intra-state sale as B2B when customer has GSTIN', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        ...
        if (isSaleItemsQuery(sql)) return [saleItemRow()];
        return [];
      });
    ...
    it('should include customer name from first+last name concatenation', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isB2bQuery(sql)) return [b2bRow({ customer_name: 'John Doe' })];
        if (isSaleItemsQuery(sql)) return [];
        return [];
      });
    ...
    it('should handle missing customer name gracefully', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isB2bQuery(sql)) return [b2bRow({ customer_name: '' })];
        if (isSaleItemsQuery(sql)) return [];
        return [];
      });
  ...
  describe('B2CS classification (small B2C aggregated by rate)', () => {
    it('should exclude large inter-state sales from B2CS (they belong to B2CL)', async () => {
      ...
      const totalB2csValue = result.b2cs.reduce((s: number, e: B2csEntry) => s + e.totalAmount, 0);
    ...
    it('should aggregate different tax rates into separate entries', async () => {
      ...
      const totalTaxable = result.b2cs.reduce((s: number, e: B2csEntry) => s + e.taxableValue, 0);
  ...
  describe('CDNR classification (credit notes from returns)', () => {
    it('should classify sale return as CDNR with negative values', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isCdnrQuery(sql)) return [cdnrRow()];
        return [];
      });
    ...
    it('should handle inter-state CDNR with IGST only', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isCdnrQuery(sql)) return [cdnrRow({ customer_state: 'Karnataka' })];
        return [];
      });
    ...
    it('should handle sale return with missing original invoice', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isCdnrQuery(sql)) return [cdnrRow({ original_invoice: null })];
        return [];
      });
  ...
  describe('generateGstr1() — full classification', () => {
    it('should return all four sections', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isB2bQuery(sql)) return [b2bRow()];
        if (isB2clQuery(sql)) return [b2clRow()];
        if (isB2csQuery(sql)) return [b2csRow()];
        if (isCdnrQuery(sql)) return [cdnrRow()];
        if (isSaleItemsQuery(sql)) return [saleItemRow()];
        return [];
      });
    ...
    it('should filter by branchId when provided', async () => {
      ...
      dataSource.query.mockImplementation((sql: string, params: any[]) => {
        ...
        if (sql.includes('branch_id =')) calledWithBranchId = true;
        return [];
      });
  ...
  describe('empty / edge case data', () => {
    ...
    it('should handle DB query failure gracefully', async () => {
      ...
      await expect(
        service.generateGstr1('2025-01-01', '2025-01-31'),
      ).rejects.toThrow();
    });
  });
  ...
  describe('generateExcel()', () => {
    ...
    it('should include summary section data in the Excel', async () => {
      dataSource.query.mockImplementation((sql: string) => {
        if (isB2bQuery(sql)) return [b2bRow(), b2bRow({ sale_id: 'sale-uuid-2', invoice_number: 'DG-MUM-2025-002', subtotal: '20000', tax_amount: '3600', total_amount: '23600' })];
        if (isB2clQuery(sql)) return [b2clRow()];
        if (isB2csQuery(sql)) return [b2csRow()];
        if (isCdnrQuery(sql)) return [cdnrRow()];
        if (isSaleItemsQuery(sql)) return [saleItemRow(), saleItemRow({ sale_id: 'sale-uuid-2' })];
        return [];
      });
