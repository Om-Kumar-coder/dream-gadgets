...
const steps = [
  ...
  {
    label: 'Build API',
    cmd: `cd ${DIR}/apps/api && npx nest build 2>&1 | tail -10`
  },
  {
    label: 'Install tsx globally',
    cmd: `npm install -g tsx 2>&1 | tail -3`
  },
  {
    label: 'Run migrations with tsx',
    cmd: `cd ${DIR}/apps/api && npx tsx run-migrations.ts 2>&1`
  },
  {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    await new Promise((resolve) => {
      conn.exec(cmd, { pty: { term: 'vt100', rows: 40, cols: 200 } }, (err, stream) => {
        if (err) { console.log(`  ERROR: ${err.message}`); resolve(undefined); return; }
        stream.on('data', (d) => process.stdout.write(d.toString()));
        stream.stderr.on('data', (d) => process.stdout.write(d.toString()));
        stream.on('close', (code) => { console.log(`  [exit: ${code}]`); resolve(undefined); });
      });
    });
    console.log('');
  }
